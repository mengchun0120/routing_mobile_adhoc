# test to prove the effectiveness and reliability of probabilistic broadcast
# arguments:
#		0		prob_
#		1		speed
#		2		index
...
Agent/PB set prob_		$val(prob)
Agent/PB set broadcastDelay_ 			0.02
Agent/PB set broadcastJitter_			0.01
...
set tracefd [open "$path/trace/test14-p$val(prob)-s$val(speed)-$val(index).tr" w]
set recfd [open "$path/trace/test14-p$val(prob)-s$val(speed)-$val(index).rec" w]
...
for {set i 0} {$i < $val(nn)} {incr i} {
	...
	set agent_($i) [new Agent/PB [$node_($i) node-addr]]
...
proc stop {} {
	...
	set asrb [expr $sum_srb / double($k)]
	
	puts $recfd "average-recv-ratio $art"
	puts $recfd "average-srb $asrb"
	
	set rd [open "$path/trace/pb-p$val(prob)-s$val(speed).txt" a]
	puts $rd "$val(index) $art $asrb"
	
	puts "existing $val(prob) $val(index)"
	close $rd
	close $tracefd
	close $recfd
	exit 0
