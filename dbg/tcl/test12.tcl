# test to get statistics, including disconnected time, average receiption ratio, average latency
# argument: 0 - member count
#						1	- speed
#						2 - scenario index
...
set tracefd [open "$path/trace/test12-m$val(member)-s$val(speed)-$val(index).tr" w]
set recfd [open "$path/trace/test12-m$val(member)-s$val(speed)-$val(index).rec" w]
...
$ns_ at 3.5 "$agent_(0) declare-group 0"
set t 2.0
